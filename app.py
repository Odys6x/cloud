...
# Flask setup for receiving data
app = Flask(__name__)
...
def receive_data():
    """Handle data received from the local processor."""
    global received_data
    received_data = request.json
    print("Received data:", received_data)  # Debug: Check data received
...
def calculate_event_gold(player_name, event_data):
    ...
    for event in event_data:
        # Ensure the event is a valid dictionary
        if not isinstance(event, dict):
            continue

        # Check event type and calculate gold
...
# Function to update the Streamlit UI
def update_ui():
    """Update Streamlit UI with the latest received data."""
    global received_data
    if not received_data.get("player_data"):
    ...
    # Calculate gold and prepare model input
    team_order_gold = sum(
    ...
    player_stats_placeholder.write("**Player Stats**")
    player_stats_placeholder.table(player_data)
    win_prob_placeholder.write(f"Game Time: {game_time}")
    win_prob_placeholder.write("Win Probabilities")
